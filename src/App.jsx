...
const db = {
  get:    (table, query="")       => sb(table + "?" + query + "&order=id"),
  insert: (table, data)           => sb(table, { method:"POST", body:JSON.stringify(data) }),
  update: (table, id, data)       => sb(table + "?id=eq." + id, { method:"PATCH", body:JSON.stringify(data), prefer:"return=representation" }),
  delete: (table, id)             => sb(table + "?id=eq." + id, { method:"DELETE", prefer:"" }),
  upsert: (table, data, onConflict) => sb(table + "?on_conflict=" + onConflict, { method:"POST", body:JSON.stringify(data), prefer:"resolution=merge-duplicates,return=representation" }),
};
...
function LoadingScreen() {
  return (
    <div style={{minHeight:"100vh",background:"#0A0F1E",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      <div style={{width:64,height:64,borderRadius:18,background:"linear-gradient(135deg,#0EA5E9,#6366F1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,marginBottom:20}}>⚡</div>
      <div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:20,color:"#E2E8F0",marginBottom:8}}>ISA Performance Dashboard</div>
      <div style={{fontSize:13,color:"#4B5E80",marginBottom:24}}>Connecting to database...</div>
      <div style={{display:"flex",gap:6}}>
        {[0,1,2].map(i=>(
          <div key={i} style={{width:8,height:8,borderRadius:"50%",background:"#0EA5E9",animation:"pulse 1.2s ease-in-out infinite",animationDelay:i*0.2+"s",opacity:0.3}}/>
        ))}
      </div>
...
function Login({ creds, appCfg, onLogin }) {
  ...
  const inp = {width:"100%",background:"#0D1424",border:"1.5px solid #1E2A45",borderRadius:10,padding:"11px 14px",color:"#E2E8F0",fontSize:15,outline:"none",fontFamily:"inherit"};
  return (
    <div style={{minHeight:"100vh",background:"#0A0F1E",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:16,fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      <style>{CSS}</style>
      <style>{`@keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-10px)}75%{transform:translateX(10px)}}`}</style>
      <div style={{width:"100%",maxWidth:400,animation:"fadeUp .5s ease"}}>
        ...
        <div style={{background:"#111827",border:"1px solid #1E2A45",borderRadius:20,padding:28,animation:shake?"shake .4s ease":"none"}}>
          <div style={{marginBottom:16}}>
            ...
            <input value={u} onChange={e=>{setU(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&attempt()} placeholder="Enter username" style={inp}/>
          </div>
          <div style={{marginBottom:20}}>
            ...
            <div style={{position:"relative"}}>
              <input type={show?"text":"password"} value={p} onChange={e=>{setP(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&attempt()} placeholder="Enter password" style={{...inp,paddingRight:44}}/>
...
function ChangeCredsModal({ creds, onSave, onClose }) {
  ...
  const save = async () => {
    ...
    setLoading(true);
    try {
      await db.update("credentials", creds.id, { username:nu.trim(), password:np||creds.password });
      onSave({ ...creds, username:nu.trim(), password:np||creds.password });
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        {ok ? (
          ...
          <div style={{marginBottom:14}}><label style={LS}>Current Password *</label><div style={{position:"relative"}}><input type={showC?"text":"password"} value={cur} onChange={e=>{setCur(e.target.value);setErr("");}} style={{...IS,paddingRight:40}} placeholder="Current password"/><button onClick={()=>setShowC(s=>!s)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#4B5E80"}}>{showC?"🙈":"👁"}</button></div></div>
          <div style={{marginBottom:14}}><label style={LS}>New Username</label><input value={nu} onChange={e=>{setNu(e.target.value);setErr("");}} style={IS} placeholder="New username"/></div>
          <div style={{marginBottom:14}}><label style={LS}>New Password (blank = keep current)</label><div style={{position:"relative"}}><input type={showN?"text":"password"} value={np} onChange={e=>{setNp(e.target.value);setErr("");}} style={{...IS,paddingRight:40}} placeholder="New password"/><button onClick={()=>setShowN(s=>!s)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#4B5E80"}}>{showN?"🙈":"👁"}</button></div></div>
          {np && <div style={{marginBottom:14}}><label style={LS}>Confirm New Password</label><input type="password" value={cp} onChange={e=>{setCp(e.target.value);setErr("");}} style={{...IS,borderColor:cp&&cp!==np?"#FCA5A5":"#1E2A45"}} placeholder="Repeat new password"/></div>}
          ...
          <div style={{display:"flex",gap:10,marginTop:6}}>
            ...
            <button onClick={save} className="btnP" style={{flex:2,opacity:loading?.6:1}}>{loading?"Saving...":"Save Changes"}</button>
          </div>
        </>)}
      </div>
    </div>
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{width:440}}>
        ...
        <div style={{background:"#0D1424",border:"1px solid #1E2A45",borderRadius:12,padding:16,marginBottom:20,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div><div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:15,color:"#E2E8F0"}}>{name||"App Name"}</div><div style={{fontSize:11,color:"#4B5E80",marginTop:2}}>Preview</div></div>
        </div>
        <div style={{marginBottom:16}}>
          ...
          <input value={name} onChange={e=>setName(e.target.value)} style={{width:"100%",background:"#0D1424",border:"1.5px solid #1E2A45",borderRadius:10,padding:"10px 14px",color:"#E2E8F0",fontSize:14,outline:"none",fontFamily:"inherit"}}/>
        </div>
        <div style={{marginBottom:24}}>
          <label style={{fontSize:11,color:"#64748B",fontWeight:700,marginBottom:10,display:"block",textTransform:"uppercase",letterSpacing:".08em"}}>Choose Logo</label>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {EMOJIS.map(e=>(<button key={e} onClick={()=>setLogo(e)} style={{width:44,height:44,borderRadius:10,background:logo===e?"linear-gradient(135deg,#0EA5E9,#6366F1)":"#1E2A45",border:logo===e?"none":"1px solid #2D3E5C",fontSize:22,cursor:"pointer"}}>{e}</button>))}
          </div>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} className="btnG" style={{flex:1}}>Cancel</button>
          <button onClick={()=>{onSave({name:name.trim()||appCfg.name,logo});onClose();}} className="btnP" style={{flex:2}}>Save</button>
        </div>
      </div>
    </div>
  );
...
function Dashboard({ creds, setCreds, appCfg, setAppCfg, onLogout }) {
  ...
  const loadAll = async () => {
    setLoading(true);
    try {
      const [ms, ls, as] = await Promise.all([
        db.get("members"),
        db.get("leads"),
        db.get("attendance"),
      ]);
      setMembers(ms || []);
      setLeads(ls || []);
      // Convert attendance array to object map
      const attMap = {};
  ...
  const setAtt = async (mid, d, s) => {
    const current = getAtt(mid, d);
    const newVal  = current === s ? null : s;
    // Optimistic update
    setAttendance(p => {
      const k = attKey(mid,d);
      if(!newVal) { const n={...p}; delete n[k]; return n; }
      return {...p, [k]: newVal};
    });
    try {
      if(!newVal) {
        await sb("attendance?member_id=eq."+mid+"&date=eq."+d, { method:"DELETE", prefer:"" });
      } else {
        await db.upsert("attendance", { member_id:mid, date:d, status:newVal }, "member_id,date");
      }
    } catch(e) { console.error("Att error:", e); loadAll(); }
  };
  ...
  const markAll = async (d, s) => {
    ...
    try {
      await db.upsert("attendance", updates, "member_id,date");
    } catch(e) { console.error("Mark all error:", e); loadAll(); }
  };
  ...
  const toggleStatus = async id => {
    ...
    setMembers(p=>p.map(x=>x.id===id?{...x,status:newStatus}:x));
    try { await db.update("members", id, { status:newStatus }); }
    catch(e) { console.error("Toggle error:", e); loadAll(); }
  };

  const softDelete = async id => {
    setMembers(p=>p.map(m=>m.id===id?{...m,deleted:true}:m));
    setConfirmDel(null);
    try { await db.update("members", id, { deleted:true }); }
    catch(e) { console.error("Delete error:", e); loadAll(); }
  };

  const restoreMember = async id => {
    setMembers(p=>p.map(m=>m.id===id?{...m,deleted:false,status:"inactive"}:m));
    try { await db.update("members", id, { deleted:false, status:"inactive" }); }
    catch(e) { console.error("Restore error:", e); loadAll(); }
  };

  const hardDelete = async id => {
    setMembers(p=>p.filter(m=>m.id!==id));
    setLeads(p=>p.filter(l=>l.member_id!==id));
    try {
      await sb("attendance?member_id=eq."+id, { method:"DELETE", prefer:"" });
      await sb("leads?member_id=eq."+id,      { method:"DELETE", prefer:"" });
      await db.delete("members", id);
    } catch(e) { console.error("Hard delete error:", e); loadAll(); }
  };
    ...
    <div style={{minHeight:"100vh",background:"#0A0F1E",fontFamily:"'DM Sans','Segoe UI',sans-serif",color:"#E2E8F0",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:"#0D1424",borderBottom:"1px solid #1E2A45",padding:"13px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}>
          {saving && <span style={{fontSize:12,color:"#0EA5E9"}}>Saving...</span>}
      ...
      <div style={{padding:"20px 24px",maxWidth:1180,margin:"0 auto",width:"100%",flex:1}}>
        ...
        {tab==="dashboard" && (
          <div>
            ...
            <div style={{...card,padding:18,marginBottom:16}}>
              <div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:14,marginBottom:12,display:"flex",alignItems:"center",gap:8}}>🗓 Today Attendance <span style={{fontSize:12,color:"#4B5E80",fontWeight:400}}>{fmtDate(today())}</span></div>
                ...
                {activeM.map(m=>{
                  ...
                  return (
                    <div key={m.id} style={{display:"flex",alignItems:"center",gap:7,background:"#0D1424",borderRadius:10,padding:"8px 12px",border:"1px solid "+(meta?meta.color+"44":"#1E2A45")}}>
                      ...
                      {meta?<span style={{background:meta.bg,color:meta.color,borderRadius:6,padding:"2px 7px",fontSize:10,fontWeight:800}}>{v}</span>:<span style={{background:"#1E2A45",color:"#4B5E80",borderRadius:6,padding:"2px 7px",fontSize:10}}>—</span>}
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",minWidth:580}}>
                <thead><tr style={{background:"#0D1424"}}>
                  {["ID","Member","Role","Status","Leads","Actions"].map(h=>(<th key={h} style={{padding:"10px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:"#4B5E80",textTransform:"uppercase",letterSpacing:".07em"}}>{h}</th>))}
                </tr></thead>
                <tbody>
                  {visibleMembers.map(m=>{
                    ...
                    return (
                      <tr key={m.id} className="row" style={{borderTop:"1px solid #1A2235"}}>
                        ...
                        <td style={{padding:"12px 16px"}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:30,height:30,borderRadius:8,background:m.color+"22",border:"2px solid "+m.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:m.color}}>{m.avatar}</div><span style={{fontWeight:600,fontSize:14}}>{m.name}</span></div></td>
                        ...
                        <td style={{padding:"12px 16px"}}><div style={{display:"flex",gap:6}}><button className="btnG" style={{fontSize:12,padding:"5px 10px"}} onClick={()=>toggleStatus(m.id)}>{m.status==="active"?"Deactivate":"Activate"}</button><button className="btnR" style={{fontSize:12,padding:"5px 10px"}} onClick={()=>setConfirmDel(m.id)}>Delete</button></div></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div>
            <div style={{...card,padding:"14px 18px",marginBottom:14}}>
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",justifyContent:"space-between"}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {Object.entries(ATT).map(([k,v])=>(<div key={k} style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:8,height:8,borderRadius:2,background:v.color}}/><span style={{fontSize:11,color:"#94A3B8"}}><b style={{color:v.color}}>{k}</b> {v.label}</span></div>))}
                </div>
            ...
            <div style={{...card,overflowX:"auto",marginBottom:14}}>
              <table style={{borderCollapse:"collapse",minWidth:660}}>
                <thead><tr style={{background:"#0D1424"}}>
                  ...
                  {mDays.map(d=>{const day=parseInt(d.split("-")[2]),dow=new Date(d).toLocaleDateString("en",{weekday:"short"}),isT=d===today();return(<th key={d} style={{padding:"6px 1px",textAlign:"center",fontSize:9,fontWeight:700,color:isT?"#0EA5E9":"#4B5E80",minWidth:44,background:isT?"#0EA5E91A":"transparent"}}><div>{dow}</div><div style={{fontSize:11,fontWeight:800}}>{day}</div></th>);})}
                          ...
                          return (
                            <td key={d} style={{padding:"2px 1px",textAlign:"center",background:isT?"#0EA5E90A":"transparent"}}>
                              <div style={{display:"flex",flexDirection:"column",gap:1,alignItems:"center"}}>
                                {Object.entries(ATT).map(([k,vm])=>(<button key={k} className="apill" onClick={()=>setAtt(m.id,d,k)} style={{background:v===k?vm.bg:"transparent",color:v===k?vm.color:"#2D3E5C",border:v===k?"1px solid "+vm.color+"55":"1px solid transparent",opacity:v===k?1:.5}}>{k}</button>))}
                              </div>
                            </td>
                          );
                        })}
                ...
                return (
                  <div key={m.id} style={{...sc,padding:"14px 15px"}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:8}}>
                      {[["P","Present",s.P,"#6EE7B7"],["A","Absent",s.A,"#FCA5A5"],["HD","Half Day",s.HD,"#FCD34D"],["L","Leave",s.L,"#C4B5FD"],["LT","Late",s.LT,"#FB923C"]].map(([k,lb,val,c])=>(<div key={k} style={{background:c+"11",border:"1px solid "+c+"33",borderRadius:7,padding:"5px 7px",textAlign:"center"}}><div style={{fontSize:15,fontWeight:700,color:c}}>{val}</div><div style={{fontSize:9,color:"#64748B",marginTop:1}}>{lb}</div></div>))}
                    </div>
                    <div style={{fontSize:10,color:"#4B5E80",display:"flex",justifyContent:"space-between",marginBottom:3}}><span>Attendance Rate</span><span style={{color:rate>=80?"#6EE7B7":rate>=50?"#FCD34D":"#FCA5A5"}}>{Math.round(rate)}%</span></div>
                    <div style={{height:5,background:"#1E2A45",borderRadius:6,overflow:"hidden"}}><div className="bfill" style={{height:"100%",borderRadius:6,background:"linear-gradient(90deg,"+m.color+","+m.color+"88)",width:rate+"%"}}/></div>
                  </div>
                );
              })}
            </div>
        ...
        {tab==="leads" && (
          <div>
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}><button className="btnP" onClick={()=>setShowAddL(true)}>+ Add Leads</button></div>
            <div style={{...card,overflow:"hidden"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr style={{background:"#0D1424"}}>
                  {["Date","Member","Role","Leads"].map(h=>(<th key={h} style={{padding:"10px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:"#4B5E80",textTransform:"uppercase",letterSpacing:".07em"}}>{h}</th>))}
                </tr></thead>
                <tbody>
                  {[...leads].filter(l=>!members.find(m=>m.id===l.member_id)?.deleted).sort((a,b)=>b.date.localeCompare(a.date)).map(l=>{
                    ...
                    return (
                      <tr key={l.id} className="row" style={{borderTop:"1px solid #1A2235"}}>
                        ...
                        <td style={{padding:"11px 16px"}}><div style={{display:"flex",alignItems:"center",gap:8}}>{m&&<div style={{width:24,height:24,borderRadius:6,background:m.color+"22",border:"2px solid "+m.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,color:m.color}}>{m.avatar}</div>}<span style={{fontWeight:500,fontSize:13}}>{m?.name||"Unknown"}</span>{m?.status==="inactive"&&<span style={{fontSize:9,color:"#FCA5A5",background:"#3B1A1A",borderRadius:10,padding:"1px 6px"}}>Inactive</span>}</div></td>
        ...
        {tab==="report" && (
          <div>
                ...
                <div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    ...
                    <div style={{marginTop:5}}>
                      ...
                      {rMode==="range"&&<><input type="date" value={rStart} onChange={e=>setRStart(e.target.value)} style={{...inp,fontSize:13,padding:"7px 10px",marginBottom:6}}/><input type="date" value={rEnd} onChange={e=>setREnd(e.target.value)} style={{...inp,fontSize:13,padding:"7px 10px"}}/></>}
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {[["both","📊 Leads & Attendance"],["leads","🎯 Leads Only"],["attendance","🗓 Attendance Only"]].map(([v,l])=>(<button key={v} className={"rbtn "+(rType===v?"rbtnA":"rbtnI")} onClick={()=>setRType(v)}>{l}</button>))}
                  </div>
                </div>
                <div>
                  ...
                  <div style={{display:"flex",gap:6,marginBottom:8}}>
                    {[["team","👥 Team"],["member","👤 Member"]].map(([v,l])=>(<button key={v} className={"rbtn "+(rTarget===v?"rbtnA":"rbtnI")} onClick={()=>{setRTarget(v);if(v==="team")setRMember("");}}>{l}</button>))}
                  </div>
                  {rTarget==="member"&&<><input placeholder="Search name or ID..." value={mSearch} onChange={e=>setMSearch(e.target.value)} style={{...inp,fontSize:12,padding:"7px 10px",marginBottom:6}}/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:11,marginBottom:16}}>
              {[{lbl:"Total Leads",val:totalLeads,c:"#93C5FD"},{lbl:"Top Performer",val:topPerf?.member.name||"—",c:"#6EE7B7",sub:(topPerf?.total||0)+" leads",sm:true},{lbl:"Period Days",val:rDates.length,c:"#FCD34D"},{lbl:"Active Staff",val:activeM.length,c:"#6EE7B7"}].map((s,i)=>(
            ...
            {rTarget==="team"&&(
              <div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                  {rType!=="attendance"&&<div style={{...card,padding:18}}><div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:13,marginBottom:12}}>🎯 Lead Performance</div>{activeSumm.map(({member:m,total})=>(<div key={m.id} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4,fontSize:13}}><span style={{fontWeight:600}}>{m.name}</span><span style={{color:m.color,fontWeight:700}}>{total}</span></div><div style={{height:7,background:"#1E2A45",borderRadius:6,overflow:"hidden"}}><div className="bfill" style={{height:"100%",borderRadius:6,background:"linear-gradient(90deg,"+m.color+","+m.color+"66)",width:(maxBar?total/maxBar*100:0)+"%"}}/></div></div>))}</div>}
                  {rType!=="leads"&&<div style={{...card,padding:18}}><div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:13,marginBottom:12}}>🗓 Attendance Summary</div>{activeSumm.map(({member:m,att,rate})=>(<div key={m.id} style={{display:"flex",alignItems:"center",gap:7,marginBottom:10}}><div style={{width:20,height:20,borderRadius:5,background:m.color+"22",border:"2px solid "+m.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:m.color,flexShrink:0}}>{m.avatar}</div><span style={{fontSize:12,fontWeight:600,flex:1}}>{m.name}</span>{[["P","#6EE7B7",att.P],["A","#FCA5A5",att.A],["HD","#FCD34D",att.HD],["L","#C4B5FD",att.L],["LT","#FB923C",att.LT]].map(([k,c,v])=>(<div key={k} style={{textAlign:"center",minWidth:22}}><div style={{fontSize:11,fontWeight:700,color:c}}>{v}</div><div style={{fontSize:8,color:"#4B5E80"}}>{k}</div></div>))}<span style={{fontSize:11,fontWeight:700,minWidth:30,textAlign:"right",color:rate>=80?"#6EE7B7":rate>=50?"#FCD34D":"#FCA5A5"}}>{rate}%</span></div>))}</div>}
                </div>
                <div style={{...card,overflow:"hidden"}}>
                    ...
                    <tbody>
                      {activeSumm.map(({member:m,total,att,rate})=>(
                        <tr key={m.id} className="row" style={{borderTop:"1px solid #1A2235"}}>
                          ...
                          {rType!=="leads"&&<><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#6EE7B7",fontSize:12}}>{att.P}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#FCA5A5",fontSize:12}}>{att.A}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#FCD34D",fontSize:12}}>{att.HD}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#C4B5FD",fontSize:12}}>{att.L}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#FB923C",fontSize:12}}>{att.LT}</td><td style={{padding:"8px 9px",textAlign:"center"}}><span style={{background:rate>=80?"#064E3B":rate>=50?"#3B2E0A":"#3B1A1A",color:rate>=80?"#6EE7B7":rate>=50?"#FCD34D":"#FCA5A5",borderRadius:6,padding:"2px 7px",fontSize:11,fontWeight:700}}>{rate}%</span></td></>}
                        </tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                {inactiveM.length>0&&<div className="inact-sec"><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><span>🔴</span><span style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:13,color:"#FCA5A5"}}>Inactive Members</span><span style={{fontSize:10,color:"#4B5E80",background:"#1E2A45",borderRadius:8,padding:"1px 8px"}}>{inactiveM.length}</span></div>
                <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:"#1A0A0A"}}><th style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:700,color:"#4B5E80",textTransform:"uppercase"}}>Member</th>{rType!=="attendance"&&<><th style={{padding:"8px 9px",textAlign:"center",fontSize:11,fontWeight:700,color:"#4B5E80",textTransform:"uppercase"}}>Leads</th><th style={{padding:"8px 9px",textAlign:"center",fontSize:11,fontWeight:700,color:"#4B5E80",textTransform:"uppercase"}}>Share</th></>}{rType!=="leads"&&<><th style={{padding:"8px 5px",textAlign:"center",fontSize:10,fontWeight:700,color:"#6EE7B7"}}>P</th><th style={{padding:"8px 5px",textAlign:"center",fontSize:10,fontWeight:700,color:"#FCA5A5"}}>A</th><th style={{padding:"8px 5px",textAlign:"center",fontSize:10,fontWeight:700,color:"#FCD34D"}}>HD</th><th style={{padding:"8px 5px",textAlign:"center",fontSize:10,fontWeight:700,color:"#C4B5FD"}}>L</th><th style={{padding:"8px 5px",textAlign:"center",fontSize:10,fontWeight:700,color:"#FB923C"}}>LT</th><th style={{padding:"8px 9px",textAlign:"center",fontSize:11,fontWeight:700,color:"#4B5E80",textTransform:"uppercase"}}>Att%</th></>}</tr></thead>
                <tbody>{inactiveSumm.map(({member:m,total,att,rate})=>(<tr key={m.id} className="row" style={{borderTop:"1px solid #3B1A1A"}}><td style={{padding:"8px 12px"}}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:20,height:20,borderRadius:5,background:m.color+"22",border:"2px solid "+m.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:m.color}}>{m.avatar}</div><div><div style={{fontSize:12,fontWeight:600,color:"#94A3B8"}}>{m.name}</div><div style={{fontSize:9,color:"#4B5E80"}}>#{m.id}</div></div><span style={{fontSize:9,background:"#3B1A1A",color:"#FCA5A5",borderRadius:6,padding:"1px 5px",fontWeight:700}}>Inactive</span></div></td>{rType!=="attendance"&&<><td style={{padding:"8px 9px",textAlign:"center",fontWeight:700,color:m.color,fontSize:14}}>{total}</td><td style={{padding:"8px 9px",textAlign:"center",color:"#64748B",fontSize:12}}>{totalLeads?Math.round(total/totalLeads*100):0}%</td></>}{rType!=="leads"&&<><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#6EE7B7",fontSize:12}}>{att.P}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#FCA5A5",fontSize:12}}>{att.A}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#FCD34D",fontSize:12}}>{att.HD}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#C4B5FD",fontSize:12}}>{att.L}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#FB923C",fontSize:12}}>{att.LT}</td><td style={{padding:"8px 9px",textAlign:"center"}}><span style={{background:rate>=80?"#064E3B":rate>=50?"#3B2E0A":"#3B1A1A",color:rate>=80?"#6EE7B7":rate>=50?"#FCD34D":"#FCA5A5",borderRadius:6,padding:"2px 7px",fontSize:11,fontWeight:700}}>{rate}%</span></td></>}</tr>))}</tbody></table></div>}
              </div>
            )}
          </div>
        )}
        ...
        {tab==="bin" && (
          <div>
            <div style={{...card,padding:"14px 18px",marginBottom:14,display:"flex",alignItems:"center",gap:10}}>
              ...
              <div><div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:14}}>Deleted Members</div><div style={{fontSize:12,color:"#4B5E80",marginTop:2}}>Restore or permanently delete. Hidden from all sections.</div></div>
            </div>
            {binMembers.length===0?(
              ...
              <div style={{...card,overflow:"hidden"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead><tr style={{background:"#0D1424"}}>{["Member","Role","All-Time Leads","Actions"].map(h=>(<th key={h} style={{padding:"10px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:"#4B5E80",textTransform:"uppercase",letterSpacing:".07em"}}>{h}</th>))}</tr></thead>
                  <tbody>
                    {binMembers.map(m=>{
                      ...
                      return (
                        <tr key={m.id} className="row" style={{borderTop:"1px solid #1A2235",opacity:.8}}>
                          <td style={{padding:"12px 16px"}}><div style={{display:"flex",alignItems:"center",gap:9}}><div style={{width:30,height:30,borderRadius:8,background:m.color+"22",border:"2px solid "+m.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:m.color}}>{m.avatar}</div><div><div style={{fontWeight:600,fontSize:14,color:"#94A3B8"}}>{m.name}</div><div style={{fontSize:10,color:"#4B5E80"}}>#{m.id}</div></div><span className="bgd">Deleted</span></div></td>
                          <td style={{padding:"12px 16px",color:"#64748B",fontSize:13}}>{m.role}</td>
                          <td style={{padding:"12px 16px",fontWeight:700,color:m.color,fontSize:16}}>{tot}</td>
                          <td style={{padding:"12px 16px"}}><div style={{display:"flex",gap:6}}><button className="btnGR" style={{fontSize:12,padding:"5px 10px"}} onClick={()=>restoreMember(m.id)}>↩ Restore</button><button className="btnR" style={{fontSize:12,padding:"5px 10px"}} onClick={()=>hardDelete(m.id)}>🗑 Delete Forever</button></div></td>
                        </tr>
                      );
                    })}
                  </tbody>
      ...
      {/* ADD MEMBER MODAL */}
      {showAddM&&(<div className="overlay" onClick={e=>e.target===e.currentTarget&&setShowAddM(false)}><div className="modal"><div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:17,marginBottom:20}}>👥 Add Team Member</div><div style={{marginBottom:13}}><label style={{fontSize:11,color:"#64748B",fontWeight:700,marginBottom:6,display:"block",textTransform:"uppercase",letterSpacing:".07em"}}>Full Name *</label><input style={inp} placeholder="e.g. John Smith" value={mName} onChange={e=>setMName(e.target.value)}/></div><div style={{marginBottom:13}}><label style={{fontSize:11,color:"#64748B",fontWeight:700,marginBottom:6,display:"block",textTransform:"uppercase",letterSpacing:".07em"}}>Role</label><input style={inp} placeholder="e.g. Sales Lead" value={mRole} onChange={e=>setMRole(e.target.value)}/></div><div style={{marginBottom:22}}><label style={{fontSize:11,color:"#64748B",fontWeight:700,marginBottom:6,display:"block",textTransform:"uppercase",letterSpacing:".07em"}}>Status</label><select style={{...inp,cursor:"pointer"}} value={mStatus} onChange={e=>setMStatus(e.target.value)}><option value="active">Active</option><option value="inactive">Inactive</option></select></div><div style={{display:"flex",gap:9}}><button className="btnG" onClick={()=>setShowAddM(false)} style={{flex:1}}>Cancel</button><button className="btnP" onClick={addMember} style={{flex:2,opacity:saving?.6:1}}>{saving?"Adding...":"Add Member"}</button></div></div></div>)}

      {/* ADD LEAD MODAL */}
      {showAddL&&(<div className="overlay" onClick={e=>e.target===e.currentTarget&&setShowAddL(false)}><div className="modal"><div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:17,marginBottom:20}}>🎯 Add Lead Entry</div><div style={{marginBottom:13}}><label style={{fontSize:11,color:"#64748B",fontWeight:700,marginBottom:6,display:"block",textTransform:"uppercase",letterSpacing:".07em"}}>Team Member *</label><select style={{...inp,cursor:"pointer"}} value={lMember} onChange={e=>setLMember(e.target.value)}><option value="">Select member...</option>{activeM.map(m=>(<option key={m.id} value={m.id}>{m.name}</option>))}</select></div><div style={{marginBottom:13}}><label style={{fontSize:11,color:"#64748B",fontWeight:700,marginBottom:6,display:"block",textTransform:"uppercase",letterSpacing:".07em"}}>Number of Leads *</label><input style={inp} type="number" min="1" placeholder="e.g. 15" value={lCount} onChange={e=>setLCount(e.target.value)}/></div><div style={{marginBottom:22}}><label style={{fontSize:11,color:"#64748B",fontWeight:700,marginBottom:6,display:"block",textTransform:"uppercase",letterSpacing:".07em"}}>Date *</label><input style={inp} type="date" value={lDate} onChange={e=>setLDate(e.target.value)}/></div><div style={{display:"flex",gap:9}}><button className="btnG" onClick={()=>setShowAddL(false)} style={{flex:1}}>Cancel</button><button className="btnP" onClick={addLead} style={{flex:2,opacity:saving?.6:1}}>{saving?"Saving...":"Save Entry"}</button></div></div></div>)}
      ...
      {/* DELETE CONFIRM */}
      {confirmDel&&(<div className="overlay" onClick={e=>e.target===e.currentTarget&&setConfirmDel(null)}><div className="modal" style={{maxWidth:360}}><div style={{fontSize:36,textAlign:"center",marginBottom:12}}>⚠️</div><div style={{fontFamily:"'Space Grotesk','DM Sans',sans-serif",fontWeight:700,fontSize:17,textAlign:"center",marginBottom:8}}>Delete Member?</div><div style={{fontSize:13,color:"#94A3B8",textAlign:"center",marginBottom:24,lineHeight:1.6}}>This member will be moved to the Bin. You can restore them anytime.</div><div style={{display:"flex",gap:9}}><button className="btnG" onClick={()=>setConfirmDel(null)} style={{flex:1}}>Cancel</button><button className="btnR" onClick={()=>softDelete(confirmDel)} style={{flex:1,fontWeight:700}}>Move to Bin</button></div></div></div>)}
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    db.get("credentials").then(rows => {
      if(rows && rows.length > 0) setCreds(rows[0]);
      else setCreds({ id:1, username:"ISA", password:"ISA@1234" });
      setLoading(false);
    }).catch(() => {
      setCreds({ id:1, username:"ISA", password:"ISA@1234" });
      setLoading(false);
    });
  }, []);
